...
import { FaCalendarAlt, FaClock, FaEye, FaPen, FaTrashAlt } from "react-icons/fa";
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase text-red-600">Admin requests</p>
            <h2 className="mt-2 text-2xl font-bold text-slate-950 sm:text-3xl">All blood donation requests</h2>
            <p className="mt-2 text-sm text-slate-500">Review, update, and manage every request from one operational view.</p>
          </div>

          <label className="form-control w-full max-w-xs">
            <span className="label-text mb-2 font-medium text-slate-700">Filter by status</span>
            <select
      ...
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        {recentDonations.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">Try changing the status filter.</p>
          </div>
        ) : (
                  ...
                  <tr key={donation._id} className="align-top">
                    ...
                    <td>
                      <div className="flex flex-wrap justify-end gap-2">
                        {donation.donationStatus === "inprogress" && (
                          <>
                            <button onClick={() => handleStatus(donation._id, "done")} className="btn btn-xs rounded-md border-emerald-600 bg-emerald-600 text-white hover:border-emerald-700 hover:bg-emerald-700">
                              Done
                            </button>
                            <button onClick={() => handleStatus(donation._id, "cancel")} className="btn btn-xs rounded-md border-slate-300 bg-white text-slate-700 hover:bg-slate-100">
                              Cancel
                            </button>
                          </>
                        )}
        ...
        <div className="modal modal-open">
          <div className="modal-box rounded-lg border border-red-100">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600">
              This action permanently removes this blood request from the queue.
            </p>
